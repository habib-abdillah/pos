...
class PenjualanDetailController extends Controller
{
    ...
    {
        $penjualanDetail = PenjualanDetail::where('id_penjualan', $request->id_penjualan)->where('kode_produk', $request->kode_produk)->first();
        if (!$penjualanDetail) {
            ...
            $jml = $detail->jumlah + 1;
            $detail->jumlah = $jml;
            $detail->subtotal = $detail->harga_jual * $jml - (($detail->diskon * $jml) / 100 * $detail->harga_jual);;
            $detail->update();
        };
    ...
    public function loadForm($diskon = 0, $potongan = 0, $total = 0, $diterima = 0)
    {
        $bayar   = $total - $potongan - ($diskon / 100 * $total);
